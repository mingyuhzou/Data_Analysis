...
plt.plot([1,2,3,4],[1,4,9,16])# 同时传入XY

# 设置XY轴的标签
plt.ylabel('Y轴')
plt.xlabel('X轴')

plt.title(''，fontsize=20)# 设置标题,第二参数设置大小

plt.xticks(rotation=90)# 做旋转防止堆叠

plt.tick_params(labelsize=14)# 设置标签大小
...
此外可以使用`markerfacecolor`参数来设置标记的填充颜色，以及`markeredgecolor`参数来设置标记的边框颜色,使用`markersize`或`ms`参数来设置标记的大小，使用`linewidth`或`lw`参数来设置线条的宽度。

```python
plt.plot([1,2,3,4],[1,4,9,16],'ro--')# 通过第三个参数设置，各种样式可以组合

plt.plot(x, y, color='red', linewidth=2, linestyle='--', marker='o', markersize=8)# 可以选择单独设置
plt.plot(x, y, 'r--o', linewidth=2, markersize=8）
         
line=plt.plot(x,y)
plt.setp(line,linewidth=0.8)# 获取返回值修改线条属性
```
...
```python
t=np.arange(0.,5.,0.2)
plt.plot(t,t,'ro--',t,t**2,'g*--',t,t**3,'b^-')# 画多条线
```

<img src="C:\Users\THINKBOOK\AppData\Roaming\Typora\typora-user-images\image-20230916105959084.png" alt="image-20230916105959084" style="zoom:50%;" />
...
```python
plt.subplot(numrows,numcols,fignum) # 在一幅图中生成多个子图，numrows表示行数 numcols表示列数，fugnum表示当前选择的图
plt.subplot(211)# 当numrows*numcols<10的时候，可以省略逗号，生成两行一列的子图，当前选择的是第一幅图
plt.subplot(212)# 切换到第二张图

plt.figure(nums)# 表示生成nums幅图,如果只生成一幅图可以省略参数
plt.figure(figsize=(10,6))# 其中figusize表示图像的尺寸
...
df=pd.read_excel('movies_.xlsx')# 读取数据

data=df['产地'].value_counts()# 筛选

plt.rcParams['font.sans-serif']=['SimHei']# 防止字体乱码

x=data.index
y=data.values

plt.figure(figsize=(20,10))# 设置图尺寸

plt.title('各国家电影产出',fontsize=25 # 标题)
plt.xlabel('国家/地区',fontsize=14)# X标签
plt.ylabel('产量',fontsize=14)# Y标签

plt.xticks(rotation=90)# 旋转，防止重叠

# 在每个柱子上方标出数字
for a,b in zip(x,y):
    plt.text(a,b,b,ha='center',va='bottom',fontsize=8,color='red')# ha设置水平对齐，va设置垂直对齐,第一二参数作为坐标，第三参数作为文本内容
...
![image-20230916120739374](C:\Users\THINKBOOK\AppData\Roaming\Typora\typora-user-images\image-20230916120739374.png)
...
df=pd.read_excel('movies_.xlsx')

data=df['产地'].value_counts()

data=df['年份'].value_counts()
...
plt.title('每年电影产出',fontsize=20)
plt.xlabel('年份',fontsize=14)
plt.ylabel('电影数目',fontsize=14)

# 由于年份众多因此每隔10位进行一次标记
for a,b in zip(x[::10],y[::10]):
    plt.text(a,b,b,ha='center',va='bottom',fontsize=10,color='red')

# 函数annotate进行箭头标记，xy给出箭头的位置，xytext给出文本的位置，arrowprops对箭头设置样式
plt.annotate('达到最大值',xy=(2010,data[2010]),xytext=(2013,13.5),arrowprops=dict(facecolor='black',edgecolor='red'))


plt.text(1980,2.3,'电影数目开始快速增长')

plt.show()
```

![image-20230916123053706](C:\Users\THINKBOOK\AppData\Roaming\Typora\typora-user-images\image-20230916123053706.png)
